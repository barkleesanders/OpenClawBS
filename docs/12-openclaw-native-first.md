...
**Context engine compat issue found during the cutover (14:02 PT fix):** The first agent turn after `openclaw models set anthropic/claude-sonnet-4-6` failed with `Context engine "lossless-claw" factory returned an invalid ContextEngine: info.id must match registered id "lossless-claw".` The gateway then fell back to `claude-cli/claude-sonnet-4-6` — visible as `model_fallback_decision: decision=candidate_failed ... reason=unknown detail=Context engine "lossless-claw" factory returned an invalid ContextEngine` at 13:56:11, then `candidate_succeeded candidate=claude-cli/claude-sonnet-4-6` at 13:56:17. The error was new as of today — zero occurrences in the prior 48 h.

Root cause: `lossless-claw@0.3.0`'s `LcmContextEngine` class does not expose an `info` property (`info.id`/`info.name`), which `openclaw@2026.4.14`'s registry contract (`dist/registry-nEl7QZ30.js:204–219`) requires. Searching `/root/.openclaw/extensions/lossless-claw/index.ts` for `this.info`, `info: {`, or `info = {` returns zero matches — the class is incompatible with the current openclaw registry contract. This bug is only visible when the anthropic/* SDK path is exercised; the claude-cli subprocess path apparently bypasses `plugins.slots.contextEngine` entirely (see the `legacy` special case at `dist/pi-embedded-runner-C72h-nWV.js:5776`). So the wrapper and the `claude-cli/*` primary were together masking this second upstream plugin bug on top of the `--resume` one.

Fix: `plugins.slots.contextEngine: "lossless-claw"` → `"legacy"` (openclaw's built-in context engine, registered at `dist/init-D2xio29-.js:38`). Gateway restarted cleanly (`[gateway] agent model: anthropic/claude-sonnet-4-6`, 0 errors in 20-sec health window). Post-fix probe: `openclaw models status --probe --probe-provider anthropic --probe-profile anthropic:oauth` returned `ok · 7.1s`, and the OAuth usage meter decremented `5h 89% left → 5h 85% left` — confirming real traffic to `api.anthropic.com/v1/messages` with the OAuth Bearer + beta headers. `auth-state.json lastGood.anthropic = "anthropic:oauth"` (which only updates on a successful completion) corroborates.

`lossless-claw` the plugin is still loaded (`[plugins] [lcm] Plugin loaded (enabled=true)`) — only the `contextEngine` *slot* was re-pointed. LCM's grep/describe tools still register. File an upstream bug on `@martian-engineering/lossless-claw` for the missing `info` property; the `legacy` slot unblocks production in the meantime.
